...
<div class="alert alert-info">
	<table cellpadding="0" cellspacing="0" border="0" class="table table-bordered" id="example" >
		...
		<tbody>
		<?php
		$senderquery = $conn->query("select distinct message_id,sender_id from message where member_id='$session_id' and delstatus=0") or die(mysql_error());
	$num_row = $senderquery->rowcount();
       ...
       <?php
    	...
    	while ($senderrow = $senderquery->fetch()) 
        {
	 	...
	 	$msgquery = $conn->query("select * from message where sender_id='$sender' and message_id='$id_msg' and delstatus=0") or die(mysql_error());
    ...
    else
    {
		...
		while ($sndrrow = $sndrquery->fetch())
         { 
		  ...
		  $sendby=$sndrrow['firstname']." ".$sndrrow['surname']." ( ".$sndrrow['access']." )";
	 ...
	 <td> <?php echo   $msgrow['status'];  ?>  </td> 
		...
		<a data-placement="top" title="Click to delete meassage" id="delete<?php echo $msg_id; ?>" href="delete_msg.php<?php echo '?del_id='.$msg_id; ?>" class="btn btn-danger"><i class="fa fa-trash-o"></i> Delete</a>
			<script type="text/javascript">
			$(document).ready(function(){
